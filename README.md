...
В результате добавления еще одного каршрутизатора в качестве сервреного устройства стали наблюдаться проблемы в лабораторном стенде. Для снижения нашрузки на стенд было принято
решение исключить использование протокола BFD. После отключения протокола производительность стенда поднялась до приемлемого уровня.
